...
If $Var(T)=\frac{1}{nI(\theta)}$, then $T$ is considered the most efficient estimator of $\theta$.
...
Let $f(t;\theta)$ be the joint PMF of PDF of $T=t(X_1,\ldots,X_n)$. The family distribution is called complete if $E_\theta\{g(T)\} = 0$ for all $\theta$ implies $P_\theta\{g(T)\} = 1$ for all $\theta$. Then $T$ is called a complete sufficient statistic.
